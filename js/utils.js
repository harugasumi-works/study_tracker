...
function weeklyMinStats(history, tracks, todayStr) {
  ...
  const daysLeft = 7 - weekDates.length;

  return tracks.map((t) => {
    const min = t.minPerWeek || 0;

    const count = weekDates.filter(
      (d) => history[d]?.[t.id]?.done
    ).length;
...
function rangeStats(history, dates, tracks) {
  ...
  return {
    perTrack,
    total,
    possible: dates.length * tracks.length,
  };
}

function categoryStats(history, dates, tracks, categories) {
  const stats = {};
  ...
  categories.forEach((cat) => {
    ...
    stats[cat] = {
      ...
      possible: dates.length * catTracks.length,
    };
  });
...
function heatColor(count, totalTracks) {
  ...
  const alpha =
    0.28 + (count / Math.max(1, totalTracks)) * 0.72;
